...
Mat  color_correction::contrast_stretching::run1(Mat src)
{
        ...
        src.copyTo(dst);
          ...
          vector<Mat> bgr_planes;
          vector<Mat> bgr_planes1;
          cv::cvtColor(src,src,COLOR_BGR2HSV_FULL);
          split( src, bgr_planes);
          split( src, bgr_planes1);
        ...
        cv::cvtColor(src,src,COLOR_HSV2BGR_FULL);
        //dst.copyTo(src);
        return dst;
    }
...
void color_correction::gray_world::preprocess(Mat src,int p,int m,float *outml, float *outma, float *outmb)
{
    Mat src1;
...
Mat  color_correction::gray_world::run2(Mat src,int p,int m)
 {
    ...
    src1.copyTo(dst);
     ...
     //clock_gettime(CLOCK_REALTIME,&finish);
     //printf("process elapsed time:  %ld\n", (finish.tv_sec* 1000000000 + finish.tv_nsec) - (start.tv_sec*1000000000 + start.tv_nsec));
...
Mat  color_correction::gray_world::run1(Mat src,int p)
{
 ...
 cvtColor(src1,src1,COLOR_BGR2Lab);
